...
<?php
 ...
 $query_createTempStuff = "create temporary table temp_searchStuff  (select fielderBibRecord.title, fielderBibRecord.ID as recordID, fielderSubjectFull.fullText as subject, fielderAuthorFull.author  from fielderBibRecord left join fielderSubjectCode on fielderBibRecord.ID = fielderSubjectCode.baseCode left join fielderSubjectFull on fielderSubjectCode.subjectID = fielderSubjectFull.subjectID left join fielderAuthorCode on fielderBibRecord.ID = fielderAuthorCode.baseCode left join fielderAuthorFull on fielderAuthorCode.authorID = fielderAuthorFull.authorID)";
...
//echo "query: " . $query_createTempStuff . "<br>";
	...
	$row_index = 0;
		...
		while ($row = $PDO_query->fetch(PDO::FETCH_ASSOC))  {
			...
			echo "<li class='alphaTitleList'><A HREF= '" . $currentHTTP . "da_catalog_titleRecord.php?ID=" . $recordID . "'>" . $title . "</a></li>";
