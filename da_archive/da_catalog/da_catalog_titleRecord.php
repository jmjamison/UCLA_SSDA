...
<?php
	...
	if (!$result) {
		...
		//print_r($subjectList_temp);
			...
			//these 2 below need to be implimented later --  120409jmj
			//echo "<A HREF='read.php'>View all saved citations</A><br>";
			//echo "<A HREF="set.php?file=I3928&src=I39281">Save Bibliographic Citation</A<br>";
		
		
			// this one I don't believe is necessary - elatorate a bit more below
			// --
	...
	// -------------------------------------------------------------------------------------------------
	//
	// 12-8-2009jmj/mike franks:  code from the viewcart.php coped in here to display the size content of the card it any thing is in it.
	//
	// --------------------------------------------------------------------------------------------------
			 if (empty($_COOKIE['ISSR_DATA_ARCHIVE_TAR'])) {
				 
			// 12-08-09jmj: do the users really need to know this??
    		//echo "Nothing stored in shopping cart, so far. ";
			echo "<br>";
 			} else {
    require("dbopen.php");
...
## loop through output list, ordering by study, then file, retrieving associated file names and titles from mysql
    ...
    while (list ($key, $val) = each ($output)) {
      ...
      if ($study <> $currentstudy) {
        $currentstudy = $study;
        $totStudies++;
         //$q = mysql_query ("select title from studytitles where study='$study'")
		//
		$q = mysql_query ("select Title from title where StudyNum='$study'")
      ...
      if (empty($file)) {
        ...
        # find all the files associated with this study because we need their info for totals, and dsn for reques
		// fix this
        $q2 = mysql_query ("select * from studyfiles where study='$study'")
          or die ("Query failed: error message = " . mysql_error ());
        while ($row2 = mysql_fetch_assoc($q2)) {
          $totFiles++;  
          $totUncompressed += $row2["size"];
          $totCompressed   += $row2["size_gzip"];
          $requestedFiles  .= $row2["file"] . "." . $row2["dsn"] . "(" . $study . ");";
        }
      } else {
		  
		 // fix this one later
        $q2 = mysql_query ("select * from studyfiles where file='$file'")
          or die ("Query failed: error message = " . mysql_error ());
...
echo "<br><h3>You have selected $totFiles file(s), in $totStudies studies, that total $totUncompressed KB storage, compressed will be approximately $totCompressed KB.</h3>";

echo "<A HREF='viewfiles.php'>View Data Cart</A><br>";
			echo "<A HREF='emptycart.php'>Empty/Clear Data Cart</A><br><br>";
			}
			
			
	// -------------------------------------------------------------------------------------------------
	//
	// end of the cart display code
	//
	// --------------------------------------------------------------------------------------------------
	#echo "<br>";
	#echo $_COOKIE['ISSR_DATA_ARCHIVE_TAR'];
	#$tempString = $_COOKIE['ISSR_DATA_ARCHIVE_TAR'];
	#print "cooking length = " . strlen($tempString);
	echo "<br>";
		...
		//	echo "To download an entire study choose: <A HREF='addfiles.php?file=&study=" . $studynumber . "&src=" . $studynumber . "'><img src='addall.gif'></a><br><br>";
			...
			echo "<th class='label'  style = 'background-color: powderblue' width=200 >File Name</th>";	
		...
		// use dtafile number rather than dsname to check for duplicates, less likely to be blank or a dup
			...
			//-------------------------------------------------------------------------------------------------------------------
			//   Lastly, if there is 1 or more Data Archive downloadable files, display the text about Add All to datacart
			//   scroll back to the beginning of the section - current around 456 to   div id='downloadallfiles'
			if ($da_downloadable >= 1) {
			$line = "<br>To download an entire study choose: <A HREF=\'addfiles.php?file=&study=" . $studynumber . "&src=" . $studynumber . "'><img src='addall.gif' alt='Add All Datasets in Study' align='absmiddle'></a>    ";
			$line .= "Downloadable dataset count = "  . $da_downloadable;
			echo "<script type=\"text/javascript\">"; 
			echo "document.getElementById('downloadallfiles').innerHTML = \"" . $line . "\";";
			echo " </script>";
			}
			//-------------------------------------------------------------------------------------------------------------------
