...
#  delete the cookie by setting the expiration date to one hour ago
  ...
  echo "Citation list cleared. Return to <a href=\"da_catalog_titleRecord.php?studynumber=$src\">Study Page</a>\n";
  exit;
}
...
if (empty($study) or empty($src)) {
  echo "Certain required parameters are missing, please email jamison@ucla.edu with this information: $REQUEST_URI and the study or file you were trying to add.\n<br>";
...
# - if no cookie present
  ...
  #   - create cookie (see format above), add file or study and give UCLA warning message with link back to $src page
   ...
   //echo "You won't be able to download these files unless you have a UCLA Login ID.<br>\n";
   echo "You have requested datasets from the Data Archive. This requires that you be logged into your UCLA BOL ID.<br>";
   echo "When you are ready to download your files, click on the 'view data cart' button on the study page and then click on the 'retrieve files' button and you will be directed to the UCLA BOL login page.<br>";
   echo "If you have any questions please contact the <a href='mailto:archivehelp@issr.ucla.edu'>data archive</a><br>";
    echo "Return to <a href=\"da_catalog_titleRecord.php?studynumber=$src\">Study Page</a>\n";
