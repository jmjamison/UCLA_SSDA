  ...
  error_reporting(0);
	...
	echo "<br>" .  $queryGetBaseRecord . "<br>";
			...
			if ($subjectListCount <= 1) {   //  just one subject/keyword
				...
				$subjectList = $subjectList[$row_index];
	...
	if (isset($_SESSION['pi_tisubsort']))
			$pi_tisubsort = $_SESSION['pi_tisubsort'];
	else 
			$pi_tisubsort = '';
...
<h1 id="siteName">UCLA Institute for Social Research Data Archives</h1> 
          <h2 id="siteName"><a href="index.html" target="_self">Maintenance Menu</a>&nbsp;/&nbsp;Edit Base Record</h2>  
</div> <!--end masthead--><!--end masthead-->

<div style="margin: 1% 5% 2% 2%;line-height: 1.5;	">
  <form <?php 
  
  	if (isset($_SESSION['studynumber'])) {     //   a study has been chosen so post means go to updateCheck
 ...
 <label>Principal Investigators (PIs) currently linked to <?php echo $studynumber;  ?>: </label> <?php echo $pi  ?><br>
         <?php if (isset($_SESSION['studynumber'])) {
					...
					//sort($piList); - already sorted
					$piListCount = count($piList);
					if (!isset($piListCount)) $piListCount = count($piList);
